...
def benfords_law_test(df, column):
    ...
    observed_first = ax[0].bar(first_digit_counts.index.astype('int64'), first_digit_freq_obs.loc[first_digit_counts.index.astype(str)], alpha=0.5, label='Observed')
    ...
    observed_two = ax[1].bar(two_digit_counts.index.astype('int64'), two_digit_freq_obs.loc[two_digit_counts.index.astype(str)], alpha=0.5, label='Observed')
    ...
    observed_three = ax[2].bar(three_digit_counts.index.astype('int64'), three_digit_freq_obs.loc[three_digit_counts.index.astype(str)], alpha=0.5, label='Observed')
