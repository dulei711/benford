...
def benfords_law_test(df, column):
    ...
    # Second position
    observed_values_2 = pd.Series(df[column].astype(str).str[:2].value_counts())
    observed_values_2 = [int(x) for x in observed_values_2]
    observed_values_2.sort()
    ...
    # Third position
    observed_values_3 = pd.Series(df[column].astype(str).str[:3].value_counts())
    observed_values_3 = [int(x) for x in observed_values_3]
    observed_values_3.sort()
