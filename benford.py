...
def benfords_law_test(df, column):
    ...
    if df[column].dtype != 'int64' and df[column].dtype != 'float64':
        st.write("The selected column is not numeric.")
        return

    first_digit_values = df[column].astype(str).str[0]
    second_digit_values = df[column].astype(str).str[1]
    third_digit_values = df[column].astype(str).str[2]

    st.write("### First Digit")
    observed_values_1 = first_digit_values.value_counts().sort_index()
    ...
    st.write("### Second Digit")
    observed_values_2 = second_digit_values.value_counts().sort_index()
    expected_values_2 = pd.Series([np.log10(1 + 1 / (10 * i + j)) for i in range(9) for j in range(10)], index=[str(i) for i in range(10)]) * len(df[column])
    ...
    st.write("### Third Digit")
    observed_values_3 = third_digit_values.value_counts().sort_index()
    expected_values_3 = pd.Series([np.log10(1 + 1 / (100 * i + 10 * j + k)) for i in range(9) for j in range(10) for k in range(10)], index=[str(i) for i in range(10)]) * len(df[column])
