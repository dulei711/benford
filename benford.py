...
def benfords_law_test(df, column):
    ...
    expected_values_2 = pd.Series([np.log10(1 + 1 / i) for i in range(1, 100)], index=[str(i) for i in range(1, 10)]) * len(df[column])
    ...
    expected_values_3 = pd.Series([np.log10(1 + 1 / i) for i in range(1, 1000)], index=[str(i) for i in range(1, 10)]) * len(df[column])
