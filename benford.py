...
def benfords_law_test(df, column):
    ...
    # Third position
    #observed_values_3 = df[column].astype(str).str[:3].value_counts().sort_index().sort_index()
    observed_values_3 = df[column].astype(str).str[:3].value_counts().sort_index(key=lambda x: x.astype(int))
