...
def benfords_law_test(df, column):
    ...
    axs[0].bar(observed_values_1, observed_values_1.values / len(df[column]), label='Observed')
    axs[0].plot(expected_values_1, expected_values_1.values / len(df[column]), 'ro-', label='Expected')
    axs[0].set_xlabel('First digit')
    axs[0].set_ylabel('Frequency')
    axs[0].set_xticks(np.arange(1, 9, step=1))
    axs[0].legend()
    ...
    axs[1].bar(observed_values_2, observed_values_2.values / len(df[column]), label='Observed')
    axs[1].plot(expected_values_2, expected_values_2.values / len(df[column]), 'ro-', label='Expected')
    ...
    axs[2].bar(observed_values_3, observed_values_3.values / len(df[column]), label='Observed')
    axs[2].plot(expected_values_3, expected_values_3.values / len(df[column]), 'ro-', label='Expected')
    axs[2].set_xlabel('Third digit')
    axs[2].set_ylabel('Frequency')
    axs[2].set_xticklabels([''] + [str(i).zfill(3) for i in range(1, 1000, 1)])
    axs[2].legend()
