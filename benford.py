...
def benfords_law_test(df, column):
    ...
    expected_values_2 = pd.Series([np.log10(1 + 1 / i) for i in range(1, 10)], index=[str(i) for i in range(0, 99)]) * len(df[column])
    ...
    expected_values_3 = pd.Series([np.log10(1 + 1 / i) for i in range(1, 10)], index=[str(i) for i in range(0, 999)]) * len(df[column])
