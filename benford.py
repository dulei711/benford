...
def benfords_law_test(df, column):
    ...
    expected_values_2 = pd.Series([np.log10(1 + 1 / i) for i in range(0, 10, 1)], index=[str(i) for i in range(0, 100, 1)]) * len(df[column])
    ...
    expected_values_3 = pd.Series([np.log10(1 + 1 / i) for i in range(0, 100, 1)], index=[str(i) for i in range(0, 1000, 1)]) * len(df[column])
